...
bool FloppyController::performIO(FloppyDevice &device, FloppyController::TransferMode mode, uint8_t *buffer, uint8_t cylinder, uint8_t head, uint8_t startSector, uint8_t sectorCount) {
    ...
    if (mode == WRITE) {
        auto sourceAddress = Util::Address<uint64_t>(buffer);
        auto targetAddress = Util::Address<uint64_t>(dmaMemory);
    ...
    for (uint8_t i = 0; i < RETRY_COUNT; i++) {
        ...
        if (mode == READ) {
            auto sourceAddress = Util::Address<uint64_t>(dmaMemory);
            auto targetAddress = Util::Address<uint64_t>(buffer);
