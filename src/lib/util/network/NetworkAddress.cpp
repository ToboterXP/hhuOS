...
NetworkAddress::NetworkAddress(uint8_t length, NetworkAddress::Type type) : buffer(new uint8_t[length]), length(length), type(type) {
    Util::Address<uint64_t>(buffer).setRange(0, length);
}
...
bool NetworkAddress::operator==(const NetworkAddress &other) const {
    ...
    auto first = Util::Address<uint64_t>(buffer);
    auto second = Util::Address<uint64_t>(other.buffer);
...
void NetworkAddress::setAddress(const uint8_t *buffer) {
    auto source = Util::Address<uint64_t>(buffer);
    auto destination = Util::Address<uint64_t>(NetworkAddress::buffer);
    destination.copyRange(source, length);
}

void NetworkAddress::getAddress(uint8_t *buffer) const {
    auto source = Util::Address<uint64_t>(NetworkAddress::buffer);
    auto destination = Util::Address<uint64_t>(buffer);
    destination.copyRange(source, length);
}
