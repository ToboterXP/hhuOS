...
String::String(const char *string) noexcept {
    auto address = Address<uint64_t>(string);
    ...
    Address<uint64_t>(buffer).copyRange(address, len);
    buffer[len] = '\0';
}

String::String(const uint8_t *data, uint32_t length) noexcept {
    auto address = Address<uint64_t>(data);
...
String::String(const String &other) noexcept {
    len = other.len;
    buffer = new char[len + 1];
    Address<uint64_t>(buffer).copyRange(Address<uint64_t>(other.buffer), len);
    buffer[len] = '\0';
}
...
String String::substring(uint32_t begin, uint32_t end) const {
    ...
    Address<uint64_t>(newBuffer).copyRange(Address<uint64_t>(this->buffer + begin), length);
    newBuffer[length] = '\0';
...
bool String::operator==(const String &other) const {
    return Address<uint64_t>(buffer).compareString(Address<uint64_t>(other.buffer)) == 0;
}
...
String &String::operator=(const String &other) {
    ...
    if (other.buffer != nullptr) {
        Address<uint64_t>(buffer).copyRange(Address<uint64_t>(other.buffer), len + 1);
    }
...
String &String::operator+=(const String &other) {
    ...
    Address<uint64_t>(buffer + len).copyRange(Address<uint64_t>(other.buffer), other.len + 1);
    len += other.len;
