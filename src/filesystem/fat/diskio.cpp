...
void* memset(void *str, int32_t c, uint32_t n) {
    Util::Address<uint64_t>(str).setRange(c, n);
    return str;
}

void* memcpy(void *dest, const void *src, uint32_t n) {
    Util::Address<uint64_t> source(src);
    Util::Address<uint64_t> target(dest);
...
int32_t memcmp(const void *str1, const void *str2, uint32_t n) {
    Util::Address<uint64_t> address1(str1);
    Util::Address<uint64_t> address2(str2);

    return address1.compareRange(address2, n);
}

char* strchr(const char *str, int c) {
    return reinterpret_cast<char*>(Util::Address<uint64_t>(str).searchCharacter(c).get());
}
